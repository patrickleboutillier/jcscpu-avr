...
jnand::jnand(input wa, input wb, output wc){
  nand x(wa, wb, wc) ;
}


jnot::jnot(input wa, output wb){
  jnand x(wa, wa, wb) ;
}


jand::jand(input wa, input wb, output wc){
  wire w ;
  jnand x(wa, wb, w) ;
  jnot y(w, wc) ;
}
...
jor::jor(input wa, input wb, output wc){
  wire wic, wid ;
  jnot n1(wa, wic) ;
  jnot n2(wb, wid) ;
  jnand x(wic, wid, wc) ;
...
jxor::jxor(input wa, input wb, output wc){
